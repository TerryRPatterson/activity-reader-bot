...
from discord.Embed import Empty
from box import Box
...
async def changeStatus(counting, start=None):
    if counting:
        activity = Activity(
            name='Counting messages',
            type=ActivityType.playing,
            state="Currently counting messages",
            start=start
            )
...
async def on_ready():
    ...
    print(bot.user.id)
    print("------")
    await changeStatus(counting=True, start=bot_start_time)
    ...
    print("Guilds loaded.")
    global messages_loaded
    messages_loaded = True
    await changeStatus(counting=False)
...
async def activity_check(context,
    ...
    if not messages_loaded:
        await context.send('I am sorry I have not finished counting the '
                           ' messages yet.')
        return
    ...
    non_posting_members = (member for member in target_guild.members
                           if member.id not in sorted_last_posts)
...
async def get_emoji(context, start: int, end: int):
    ...
    async with aiohttp.ClientSession() as session:
        async for message in target_channel.history(limit=None, before=before,
            ...
            for attachment in embed.attachments:
                await upload_emoji(
                    attachment.url,
                    session,
                    context.guild,
...
dir = Path.home() / 'emojis'


def emoji_url(id, name):
    return f'https://cdn.discordapp.com/emojis/{id}.{type}'


async def upload_emoji(url, session, guild, type='png'):
    async with session.get(url) as resp:
        if resp.status == 200:
            with open(dir / f'{name}.{type}') as file:
            while True:
